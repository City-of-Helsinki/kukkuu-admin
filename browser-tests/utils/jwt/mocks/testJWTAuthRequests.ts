...
export class AuthServiceRequestInterceptor extends RequestHook {
   ...
   *
   */
  constructor(user: OIDCUserProfileType = browserTestAdminUser) {
  ...
  async fetchOIDCEndpointsConfiguration() {
    ...
    // eslint-disable-next-line no-console
    console.debug(`Fetch OIDC configuration from ${oidcConfigurationEndpoint}`);
    return fetch(oidcConfigurationEndpoint).then(
      (response) =>
