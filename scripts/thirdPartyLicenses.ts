 ...
 *
 * This application has no Sass pipeline and no CSS-only dependencies -- its
 * styles come from its own CSS modules and from Emotion at runtime -- so the
 * list is empty. Add a package here if one is ever imported from a stylesheet.
 */
const SASS_RESOLVED_PACKAGES: string[] = [];
 ...
 * and this only records the identifier that matches that text. Add an entry
 * only after reading the package's own license file.
 */
const LICENSE_CORRECTIONS: Record<string, LicenseOverride> = {
  'css-mediaquery': {
    license: 'BSD-3-Clause',
    note:
      'Declares the ambiguous, non-SPDX string "BSD" in its package.json. ' +
      'Its license file is the three-clause BSD license of Yahoo! Inc., ' +
      'reproduced in full below, so BSD-3-Clause is the identifier that ' +
      'matches the text that governs.',
  },
};
...
export function thirdPartyLicenses(): Plugin {
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      // is needed on top of them because a CSS-only package never shows up
      // in a JavaScript chunk even though its styles ship.
